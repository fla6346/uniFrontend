...
const Contenido = () => {
  ...
  const [imagenEventoObjeto, setImagenEventoObjeto] = useState(null);
  ...
  const seleccionarImagen = async () => { let result = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ImagePicker.MediaTypeOptions.Images, allowsEditing: true, aspect: [16, 9], quality: 0.7, }); if (!result.canceled && result.assets && result.assets.length > 0) { setImagenEvento(result.assets[0].uri); setImagenEventoObjeto(result.assets[0]); } };
      ...
      console.log(`[DEBUG] Valor del authToken: ${authToken}`); 
        if (!authToken) {
           ...
           formData.append('horaevento', formatToISOTime(fechaHoraSeleccionada));
            ...
            if (imagenEventoObjeto && imagenEventoObjeto.uri) {
               if (Platform.OS === 'web') { 
                formData.append('imagenEventoBase64', imagenEventoObjeto.uri); } 
                else { 
                  const uri = imagenEventoObjeto.uri; 
                  const fileObject = { uri: uri, name: imagenEventoObjeto.fileName || uri.split('/').pop(), type: imagenEventoObjeto.mimeType || 'image/jpeg', }; 
    ...
    >
      ...
      >
        ...
        <View style={styles.formSection}>
          ...
          <TouchableOpacity onPress={seleccionarImagen} style={styles.imagePickerButton}>
            {imagenEvento ? <Image source={{ uri: imagenEvento }} style={styles.imagePreview} />
             : <View style={styles.imagePlaceholder}><Ionicons name="image-outline" size={50} color="#ccc" /><Text style={styles.imagePlaceholderText}>Seleccionar Imagen</Text></View>}
          </TouchableOpacity>
