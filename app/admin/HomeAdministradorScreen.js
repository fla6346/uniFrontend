  ...
  return (
    <View style={styles.expandableBannerContainer}>
      ...
      >
        ...
        <Pressable
          onPress={onLogout}
          style={styles.expandedLogoutButton}
        >
...
const HomeAdministradorScreen = () => {
  ...
  const [loading, setLoading] = useState(false); // Mantener por si se usa en el futuro
  const [loadingNotifications, setLoadingNotifications] = useState(false);
  const [activeUsersCount, setActiveUsersCount] = useState('0'); // Mantener por si se usa en el futuro
  ...
  let cardWidth = (windowWidth - CARD_MARGIN * columns * 2) / columns;
  ...
  const renderNotification = ({ item }) => (
    <TouchableOpacity
      ...
      onPress={() => handleNotificationPress(item)} // --- MODIFICACIÓN AQUÍ: Pasar el item completo ---
    >
      <View style={[
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {/* Nuevo banner de notificaciones simplificado */}
        {unreadCount > 0 && (
          <TouchableOpacity
      ...
      {/* Banner expansible en la parte inferior */}
