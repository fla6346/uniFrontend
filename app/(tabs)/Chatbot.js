...
// Sala general para todos los usuarios
...
export default function ChatScreen() {
  ...
  const [usuario, setUsuario]       = useState(null); // {id, nombre, role}

  const socketRef   = useRef(null);
  const flatListRef = useRef(null);

  // ─── Cargar usuario guardado en sesión ───────────────────────────
  useEffect(() => {
  ...
  // ─── Conectar Socket.io cuando ya tenemos el usuario ─────────────
  useEffect(() => {
    if (!usuario) return;
  ...
  }, [usuario]); // se reconecta si cambia el usuario
